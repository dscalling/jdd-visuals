...
function openGallery(i){
  ...
  galleryGrid.innerHTML='<div class="gallery-loading">GALERIE WIRD GELADEN …</div>';
  galleryModal.hidden=false;galleryModal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');

  const candidates=[];
  for(let n=1;n<=18;n++){
    const base=`assets/gallery/${folder}/${String(n).padStart(2,'0')}`;
    for(const ext of ['jpg','jpeg','png','webp']) candidates.push(`${base}.${ext}`);
  }
  let pending=candidates.length,loaded=0;
  const loadedImages=[];
  candidates.forEach(src=>{
    const probe=new Image();
    probe.onload=()=>{
      loaded++;
      loadedImages.push({src,order:src});
      finishOne();
    };
    probe.onerror=finishOne;
    probe.src=src;
  });
  function finishOne(){
    pending--;
    if(pending!==0)return;
    loadedImages.sort((a,b)=>a.src.localeCompare(b.src,undefined,{numeric:true}));
...
(()=>{
  ...
  addEventListener('load',()=>setTimeout(()=>{if(loader)loader.classList.add('loaded')},500));
  ...
  if(!reduce&&matchMedia('(pointer:fine)').matches){
    ...
    const art=$('.hero-art');addEventListener('pointermove',e=>{const px=(e.clientX/innerWidth-.5)*12,py=(e.clientY/innerHeight-.5)*9;art.style.transform=`translate(${px}px,${py}px)`});
